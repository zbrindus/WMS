...
def mutare(loc):
    # Facturile scanate se muta in folderul unde vor fi prelucrate si dupa prelucrare se muta inapoi in folderul de unde au fost luate.
    if loc == "IN":
        source = "//apollo2/eta/Logistica/Depozit/Facturi/Scan"
        dest = rootDir()
    if loc == "OUT":
        source = rootDir()
        dest = "//apollo2/eta/Logistica/Depozit/Facturi/Scan"
...
def redenumire(elem, new_dict, img_to_ocr):
    ...
    ocr_config = shlex.quote("--psm 6 --oem 3 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 ")
...
def redenumire_main(eFactura=True):
    # Alegem directorul în care lucrăm.
    d = os.getcwd()
    # Deschidem fișierul json.
    if eFactura == True:
        with open("efactura.json") as json_file:
    ...
    for i in range(len(pdf_files)):
        ...
        with tempfile.NamedTemporaryFile(mode="wb"):
            ...
            crop_box = (0, 0, 1900, 970) # ACESTA ESTE VERSIUNEA ORIGINALA SI ABSOLUT FUNCTIONALA !!!!!!!!
            # crop_box = (0, 0, 1900, 2070) # ACEASTA ESTE VERSIUNEA PENTRU CRAYON, CAND SUNT MULTE FACTURI !!!!!!!!
...
def rotate():
    """ Daca este eFactura, atunci trebuie pusa in pozitia landscape. """

    d = rootDir() + "\\dist"
    # Creăm o listă care conține toate fișierele cu extensia .pdf din folder.
    pdf_files = [rootDir() + "\\" + pdfFile for pdfFile in os.listdir(rootDir()) if pdfFile.endswith(".pdf")]
    count = 0
    for file in pdf_files:
        ...
        pdfOut = open("eFactura_" + str(count) + ".pdf", "wb")
        pdfWriter.write(pdfOut)
        pdfOut.close()
        pdfIn.close()
        count += 1

    # Stergem fisierele pdf pe care le-am intors.
    for pdfFile in os.listdir(rootDir()):
        if pdfFile.endswith(".pdf"):
            os.remove(rootDir() + "\\" + pdfFile)

    # Mutam fisierele intoarse in locul celor de dinainte.
    for file in os.listdir(d):
        if file.endswith(".pdf"):
            shutil.move(file, rootDir())
...
def extern_main(name):
    ...
    if name == "DELL":
        box = (0, 0, 1900, 970)
        psm = shlex.quote("--psm 6")
        invoice_no = r"Invoice No\s*:\s*(\d*)"
        order_no = r"Order No\s*:\s*(\d*)"

    if name == "HP":
        box = (700, 680, 1200, 780)
        psm = shlex.quote("--psm 3")
    ...
    for i in range(len(pdf_files)):
        ...
        with tempfile.NamedTemporaryFile(mode="wb"):
            ...
            # Transformăm imaginea în text.
            conf = shlex.quote(" --oem 3 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 ")
            ocr_config = psm + conf
            text = pytesseract.image_to_string(img_file, config=ocr_config)
            # print(text)
...
######################################### SCANARE CODURI DE BARA #############################################

def scanare_coduri_de_bara(n):
    ...
    # Scanam doar codurile de bare care au o lungime mai mare de ...
    for j in range(len(to_decode)):
        a = decode(PIL.Image.open(to_decode[j]))
        for elem in a:
            with open("serii.txt", "a") as file1:
                serie = elem[0].decode("utf-8")
                if len(serie) > int(n):
                    file1.write(serie + "\n")
    ...
    for imgFile in os.listdir(d):
        if imgFile.endswith(".jpg"):
            os.remove(imgFile)
    # Mutam fisierul cu seriile scanate.
    shutil.move("serii.txt", rootDir())
